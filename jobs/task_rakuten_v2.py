...
@click.command('task_rakuten_v2', help="Hello World.") 
@with_appcontext
def task_rakuten_v2_run():

    # 処理日
    process_date = datetime.datetime.now().strftime('%Y%m%d%H%M')
    # 出力ファイル名
    file_name='research_rakuten_v2'+process_date+'.tsv'
    ...
    for rnk in rnk_list:
        ...
        shohin_list = [] 
        for elem in elems:
            shohin_list.append(elem.find_element(By.TAG_NAME, "a").get_attribute("href"))

        # 結果リスト
        result_list=list()
        for shohin in shohin_list:
            result_list.append({'invalid':0, 'item_url':shohin})
